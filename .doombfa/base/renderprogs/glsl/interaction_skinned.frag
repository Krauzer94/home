// filename renderprogs/interaction.ps.hlsl
#version 460
...
void main() {
	...
	vec3 diffuseColor = diffuseMap * sRGBToLinearRGB ( _fa_[0 /* rpDiffuseModifier */] . xyz ) ;
	vec3 specularColor = specMap. xyz * specularContribution * sRGBToLinearRGB ( _fa_[1 /* rpSpecularModifier */] . xyz ) ;
	vec3 lightColor = sRGBToLinearRGB ( lightProj. xyz * lightFalloff. xyz ) ;
	fo_FragColor . xyz = ( diffuseColor + specularColor ) * lambert * lightColor * vofi_Color . rgb ;
	fo_FragColor . w = 1.0 ;
}