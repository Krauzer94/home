...
void main() {
	...
	for ( int i = 0 ; i < 4 ; i ++ )
	{
		if ( viewZ < _fa_[5 /* rpCascadeDistances */] [ i ] )
		{
			shadowIndex = i ;
			break ;
		}
	}
	vec4 shadowMatrixX = _fa_[/* rpShadowMatrices */ 6 + int ( shadowIndex * 4 + 0 ) ] ;
	vec4 shadowMatrixY = _fa_[/* rpShadowMatrices */ 6 + int ( shadowIndex * 4 + 1 ) ] ;
	vec4 shadowMatrixZ = _fa_[/* rpShadowMatrices */ 6 + int ( shadowIndex * 4 + 2 ) ] ;
	vec4 shadowMatrixW = _fa_[/* rpShadowMatrices */ 6 + int ( shadowIndex * 4 + 3 ) ] ;
